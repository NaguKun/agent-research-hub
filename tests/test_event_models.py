...
class TestEventTypes:
    ...
    def test_session_start(self, sample_context):
        event = AgentEvent(
            ...
            payload=SessionStartPayload(session_id="sess-001", message="Research AI"),
        )
    ...
    def test_thinking(self, sample_context):
        event = AgentEvent(
            ...
            agent_context=sample_context,
            payload=ThinkingPayload(text="Analyzing the request..."),
        )
    ...
    def test_agent_response(self, sub_context):
        event = AgentEvent(
            ...
            payload=AgentResponsePayload(text="Here are the results..."),
        )
    ...
    def test_ask_user_answered(self, sample_context):
        event = AgentEvent(
            ...
            agent_context=sample_context,
            payload=AskUserAnsweredPayload(prompt_id="prompt-001", answer="Developer adoption"),
        )
    ...
    def test_done(self, sample_context):
        event = AgentEvent(
            ...
            agent_context=sample_context,
            payload=DonePayload(run_id="run-001", total_events=42),
        )
...
class TestEventSerialization:
    def test_json_round_trip(self, sample_context):
        event = AgentEvent(
            event_type=EventType.THINKING,
            agent_context=sample_context,
            payload=ThinkingPayload(text="Processing..."),
            run_id="run-001",
        )
    ...
    def test_event_has_auto_generated_fields(self, sample_context):
        event = AgentEvent(
            event_type=EventType.THINKING,
            agent_context=sample_context,
            payload=ThinkingPayload(text="test"),
        )
